...
void fillRandomPattern(std::vector<T>& v, std::uint32_t stride) {
    ...
    for (std::uint32_t j = 0; j < v.size(); ++j) {
        ...
        while (((newpos >= pos) && (newpos <= pos + stride)) ||
               ((newpos <= pos) && (newpos >= pos - stride))) {
            newpos = dis(gen);
        }
        v[pos] = newpos;
        pos = newpos;
...
void saveGraphs(const std::vector<uint32_t>& sizes, const std::vector<uint32_t>& strides,
    ...
    for (std::uint32_t np = 0; np < numgraphs; ++np) {
        ...
        int pixstride =
            Cairo::ImageSurface::format_stride_for_width(Cairo::FORMAT_ARGB32, height);
        Cairo::RefPtr<Cairo::ImageSurface> surface = Cairo::ImageSurface::create(
            (std::uint8_t*)pixels.data(), Cairo::FORMAT_ARGB32, height, width, pixstride);
...
enum FillPattern : std::uint8_t { Sequential, Random };

template <typename Policy>
void calcResults() {
    const std::vector<uint32_t> sizes =
        genLogRange(Policy::SizeRange[0], Policy::SizeRange[1], Policy::SizeRatio);
    const std::vector<uint32_t> strides =
        genLogRange(Policy::StrideRange[0], Policy::StrideRange[1], Policy::StrideRatio);
    std::vector<uint32_t> values(sizes.size() * strides.size() *
                                 Policy::Percentiles.size());
    ...
    for (std::uint32_t size : sizes) {
        for (std::uint32_t stride : strides) {
            Histogram<Policy::HistogramBins> hist(Policy::HistogramRange[0],
                                                  Policy::HistogramRange[1]);
            hist.clear();
            if (size > 2 * stride) {
            ...
            std::cout << "Policy," << Policy::Name << ",Stride," << stride << ",Size,"
                      << size << ",";
