...
//! Computes the approximation of the inverse CDF for a given p-value
static inline double invcdf(double p) {
    ...
    if (p < 0.5) return -approx(std::sqrt(-2.0 * std::log(p)));
...
class Histogram {
public:
    Histogram(double min, double max) : _min(min), _max(max) {
        clear();
    }
    void clear() {
        for (Bin& bin : _bins) {
    ...
    void add(double value, std::uint64_t count = 1) {
        ...
        if (value < _min)
            ...
            idx = ((value - _min) / (_max - _min)) * NUMBINS;
        Bin& bin(_bins[idx]);
        bin.sum.add(value);
