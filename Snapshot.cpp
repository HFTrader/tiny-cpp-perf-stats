...
Snapshot::Snapshot(int debuglevel)
    : cycles(PERF_TYPE_HARDWARE, PERF_COUNT_HW_CPU_CYCLES),
      ...
      tlbmisses(PERF_TYPE_HW_CACHE, PERF_COUNT_HW_CACHE_DTLB | PERF_COUNT_HW_CACHE_OP_READ << 8 | PERF_COUNT_HW_CACHE_RESULT_MISS << 16),
      debug(debuglevel)
{
}
...
Snapshot::Sample Snapshot::stop(const std::string &evname, uint64_t numitems, uint64_t numiterations)
{
    ...
    if (debug >= 2)
        std::cout << "Statistics: " << numitems << " items, " << numiterations << " iterations. Per item:"
                  << human(samp.cycles,3) << " cycles, " << human(samp.instructions, 3) << " instructions, "
                  << human(samp.cachemisses, 3) << " cache misses, " << human(samp.branchmisses, 3)
                  << " branch misses, " << human(samp.tlbmisses, 3 ) << " TLB misses\n";
...
void Snapshot::summary(const std::string &header, FILE *f)
{
    ...
    static const std::vector<Metric> metrics = { {"Constant",     [](const Sample& s) { return 1;}},
                                                 {"CacheMisses",  [](const Sample& s) { return s.cachemisses;}},
                                                 {"BranchMisses", [](const Sample& s) { return s.branchmisses;}},
                                                 { "TLBMisses",   [](const Sample& s) { return s.tlbmisses;}},
                                                 { "Log(N)",      [](const Sample& s) { return log(s.numitems); }},
                                                 { "N",           [](const Sample& s) { return s.numitems; }},
                                                 { "N*Log(N)",    [](const Sample& s) { return s.numitems*log(s.numitems); }},
                                                 { "N^2",         [](const Sample& s) { return s.numitems*log(s.numitems); }} 
    };
